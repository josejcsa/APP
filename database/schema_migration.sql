...
-- Suporta perfis: Admin Geral, Técnico de Campo e Cliente
-- -------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `usuarios_auth` (
  ...
  -- Índices de Alta Performance para Login e Autenticação
...
-- Trilha imutável de ações para conformidade e segurança
-- -------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `auditoria_historico` (
  `id` INT AUTO_INCREMENT PRIMARY KEY,
  `guid` VARCHAR(64) UNIQUE NOT NULL,
  `entity_type` VARCHAR(50) NOT NULL COMMENT 'checklist, appointment, contact, financial, settings, auth',
  `entity_id` VARCHAR(64) NOT NULL,
  `acao` VARCHAR(50) NOT NULL COMMENT 'Criação, Edição, Exclusão, Visualização, Assinatura, Sincronização, Login',
  ...
  -- Índices Otimizados para Consultas Rápidas e Filtros
...
-- PWA Offline-First buffer e resolução de conflitos
-- -------------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS `cadastros_contatos` (
  ...
  `nome` VARCHAR(255) NOT NULL,
  `documento` VARCHAR(50) DEFAULT NULL COMMENT 'CPF ou CNPJ',
...
-- Senha: Hash padrão compatível com autenticação Elthera
-- -------------------------------------------------------------------------
